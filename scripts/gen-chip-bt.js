...
const assets = [
    {
        name: 'part_24_chip',
        prompt:
            'Cute cartoon 3D small colorful microchip semiconductor, '
            + 'square shape with rounded corners, bright pastel green surface, '
            + 'cute simplified golden circuit traces on top, '
            + 'NO pins, NO legs, NO protruding parts on the sides, '
            + 'clean smooth edges, flat and simple like a sticker or tile, '
            + 'friendly toy-like, very cute and clean, '
            + 'single isolated small item, centered, '
            + BASE
            + ', transparent background',
    },
    {
        name: 'part_21_bluetooth',
        prompt:
            'Cute cartoon 3D small bluetooth module circuit board, '
            + 'tiny rectangular dark blue PCB with a small bluetooth antenna, '
            + 'white bluetooth symbol printed on the board, '
            + 'a few tiny colorful electronic components on the surface, '
            + 'small and compact wireless device, '
            + 'single isolated small item, centered, '
            + BASE
            + ', transparent background',
    },
...
(async () => {
    const dir = path.resolve('public/assets/generated/parts');
    ...
    for (const asset of assets) {
        ...
        if (fs.existsSync(finalPath)) {
            const base = path.basename(finalPath, '.png');
            fs.copyFileSync(finalPath, path.join(archDir, base + '_prev.png'));
        }
