...
(async () => {
    const prompt =
        'Cute cartoon 3D simple plain metal bolt and nut, '
        + 'ordinary silver gray hexagonal bolt head with a short shaft and nut, '
        + 'plain and simple hardware, no gems, no gold, no decorations, '
        + 'just a regular cartoon bolt, single small item, centered, '
        + BASE
    ...
    // Archive old crystal_crown
    const old = path.join(dir, 'part_20_crystal_crown.png');
    if (fs.existsSync(old)) fs.copyFileSync(old, path.join(archDir, 'part_20_crystal_crown.png'));

    const rawPath = path.join(dir, 'part_20_bolt_raw.png');
    const finalPath = path.join(dir, 'part_20_bolt.png');

    console.log('Generating bolt...');
