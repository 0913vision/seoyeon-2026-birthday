...
async function download(assetId, out) {
    const d = await apiCall('GET', ROOT + '/assets/' + assetId); const url = d.asset?.url; const resp = await fetch(url); fs.writeFileSync(out, Buffer.from(await resp.arrayBuffer()));
}

(async () => {
    const prompt = 'Cute cartoon 3D small rolled dark charcoal leather roll icon, single small leather roll tied with string, resource icon style, centered, ' + BASE + ', transparent background';
    console.log('gen leather icon');
    ...
    console.log(' ok');
    const rawPath = path.resolve('public/assets/generated/resources/leather_raw.png');
    const finalPath = path.resolve('public/assets/generated/resources/leather.png');
