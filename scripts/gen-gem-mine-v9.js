...
(async () => {
    const prompt =
        'Cute cartoon 3D crystal mining station on grass tile, '
        + 'a small conveyor belt carrying mined purple crystals to one side, '
        + 'ONE large purple crystal lying horizontally on its side tied down with thick ropes, '
        + 'a tall wooden crane with ropes lifting and holding the large crystal, '
        + 'the crane is actively hoisting the big crystal in the air, '
        + 'isometric view, sitting on grass, '
        + BASE
        + ', transparent background';

    const dir = path.resolve('public/assets/generated/buildings');
    const rawPath = path.join(dir, 'gem_cave_v9_raw.png');
    const finalPath = path.join(dir, 'gem_cave_v9.png');

    console.log('Generating v9...\n');
